...
for lr in "${lrs[@]}"; do
    for seed in "${seeds[@]}"; do
        if $use_mup = "true"; then
            torchrun --nproc_per_node "$nproc_per_node" --no-python datascaler-datacomp train --scale "$scale" --data-dir "$data_dir" --output-dir "$output_dir" --exp-name "${scale}_${width_mult}_${min_width_mult}_${lr}_${attn_mult}_${output_mult}_${use_mup}_${method}_${min_fraction}_${fraction}_${model_arch}_${seed}" \
            ...
            --attn-mult "$attn_mult" --output-mult "$output_mult" --log-every-n-steps "$log_every_n_steps" --val-ratio "$val_ratio" --val-batch-size "$val_batch_size" --lr-scheduler "$lr_scheduler" --subset-file "${subset_file_path}/${method}-${scale}/${scale}_${method}_${min_fraction}_${fraction}_trie.pkl" --use-mup
        else
            torchrun --nproc_per_node "$nproc_per_node" --no-python datascaler-datacomp train --scale "$scale" --data-dir "$data_dir" --output-dir "$output_dir" --exp-name "${scale}_${width_mult}_${min_width_mult}_${lr}_${attn_mult}_${output_mult}_${use_mup}_${method}_${min_fraction}_${fraction}_${model_arch}_${seed}" \
            --seed "$seed" --width-mult "$width_mult" --min-width-mult "$min_width_mult" --train-num-samples "$train_num_samples" --num-checkpoints "$num_checkpoints" --save-frequency "$save_frequency" --learning-rate "$lr" --workers "$workers" --model-arch "$model_arch" \
            --attn-mult "$attn_mult" --output-mult "$output_mult" --log-every-n-steps "$log_every_n_steps" --val-ratio "$val_ratio" --val-batch-size "$val_batch_size" --lr-scheduler "$lr_scheduler" --subset-file "${subset_file_path}/${method}-${scale}/${scale}_${method}_${min_fraction}_${fraction}_trie.pkl" --no-use-mup
        fi
    done
done
