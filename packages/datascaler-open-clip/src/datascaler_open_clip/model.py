...
from datascaler_open_clip.transformers import (
    Attention,
    LayerNorm,
    LayerNormFp32,
    QuickGELU,
...
class CLIPTextCfg:
    ...
    output_tokens: bool = False
    act_kwargs: dict = None  # type: ignore
    norm_kwargs: dict = None  # type: ignore
...
def _build_vision_tower(
    ...
    if vision_cfg.timm_model_name:
        ...
        visual = ModifiedResNet(
            layers=vision_cfg.layers,  # type: ignore
            output_dim=embed_dim,
            heads=vision_heads,
            image_size=vision_cfg.image_size,  # type: ignore
        ...
        visual = VisionTransformer(
            ...
            ls_init_value=vision_cfg.ls_init_value,  # type: ignore
...
def _build_text_tower(
    ...
    if text_cfg.hf_model_name:
        ...
        text = TextTransformer(
            ...
            ls_init_value=text_cfg.ls_init_value,  # type: ignore
            output_dim=embed_dim,
...
class CLIP(nn.Module):
    output_dict: torch.jit.Final[bool]  # type: ignore
    ...
    def __init__(
        ...
        self.register_buffer("attn_mask", text.attn_mask, persistent=False)  # type: ignore
    ...
    def no_weight_decay(self):
        ...
        if hasattr(self.visual, "no_weight_decay"):
            for n in self.visual.no_weight_decay():  # type: ignore
                no_wd.add("visual." + n)
        return no_wd
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        x = self.token_embedding(text).to(cast_dtype)  # [batch_size, n_ctx, d_model] # type: ignore

        x = x + self.positional_embedding.to(cast_dtype)
        x = self.transformer(x, attn_mask=self.attn_mask)
        x = self.ln_final(x)  # [batch_size, n_ctx, transformer.width] # type: ignore
        ...
        if self.text_projection is not None:
            if isinstance(self.text_projection, (nn.Linear, MuReadout)):
                ...
                x = x @ self.text_projection  # type: ignore
    ...
    def forward_intermediates(
        ...
        if image is not None:
            ...
            if normalize and "image_features" in image_output:
                image_output["image_features"] = F.normalize(
                    image_output["image_features"],  # type: ignore
                    dim=-1,
                )
        ...
        if text is not None:
            cast_dtype = self.transformer.get_cast_dtype()
            x = self.token_embedding(text).to(  # type: ignore
                cast_dtype
            )  # [batch_size, n_ctx, d_model]
            x = x + self.positional_embedding.to(cast_dtype)
            x, intermediates = self.transformer.forward_intermediates(
                x,
                attn_mask=self.attn_mask,  # type: ignore
                indices=text_indices,
            )
            if normalize_intermediates:
                intermediates = [self.ln_final(xi) for xi in intermediates]  # type: ignore
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(x):
        if isinstance(x, (nn.Conv1d, nn.Conv2d, nn.Linear)):
            x.weight.data = x.weight.data.to(dtype)
            if x.bias is not None:
                x.bias.data = x.bias.data.to(dtype)

        if isinstance(x, (nn.MultiheadAttention, Attention)):
            for attr in [
        ...
        if isinstance(x, (CLIP, TextTransformer)):
            # convert text nn.Parameter projections
            attr = getattr(x, "text_projection", None)
            if attr is not None:
                attr.data = attr.data.to(dtype)

        if isinstance(x, VisionTransformer):
            # convert vision nn.Parameter projections
            attr = getattr(x, "proj", None)
            if attr is not None:
                attr.data = attr.data.to(dtype)
...
def build_model_from_openai_state_dict(
    ...
    vision_cfg = CLIPVisionCfg(
        ...
        width=vision_width,
        patch_size=vision_patch_size,  # type: ignore
...
def resize_pos_embed(state_dict, model, interpolation: str = "bicubic", antialias: bool = True):
    ...
    new_seq_len = grid_size[0] * grid_size[1] + extra_tokens  # type: ignore
    ...
    pos_emb_img = pos_emb_img.reshape(
        1,
        old_grid_size[0],  # type: ignore
        old_grid_size[1],  # type: ignore
        -1,
    ).permute(0, 3, 1, 2)
    pos_emb_img = F.interpolate(
        pos_emb_img,
        size=grid_size,  # type: ignore
        mode=interpolation,
        antialias=antialias,
        align_corners=False,
    )
...
def resize_text_pos_embed(
    ...
    num_pos = model_pos_embed.shape[0]  # type: ignore
    width = model_pos_embed.shape[1]  # type: ignore
