...
def evaluate_one_epoch(
    ...
    if is_master(args):
        logging.info(f"Eval Epoch: {epoch}, Loss: {avg_loss}, total_samples: {total_samples}")
...
def evaluate(model, data, epoch, args, tb_writer=None, tokenizer=None):
    ...
    if "val" in data and (
        ...
        with torch.inference_mode():
            ...
            # )
            print(cumulative_loss, num_samples)
            loss = cumulative_loss / num_samples
            metrics.update(
                {
                    # **val_metrics,
                    "clip_val_loss": loss.item(),  # type: ignore
                    "epoch": epoch,
            ...
            if gen_loss is not None:
                ...
                metrics.update({"val_generative_loss": gen_loss.item()})  # type: ignore

    if not metrics:
        return metrics

    logging.info(
        f"Eval Epoch: {epoch} " + "\t".join([f"{k}: {round(v, 4):.4f}" for k, v in metrics.items()])
    )
