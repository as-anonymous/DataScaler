...
try:
    ...
    try:
        ...
        deform_conv_cpu = cpp_extension.load(
            ...
            sources=[
                os.path.join(dcn_dir, "src", "deform_conv_cpu.cpp"),
...
if torch.cuda.is_available():
    try:
        ...
        warnings.warn(
            "Using precompiled deform_conv_cuda from {}".format(deform_conv_cuda.__file__)
        )
        dcn_cuda_ready = True
    except:
        try:
            ...
            warnings.warn("(This may take a while if this module is loaded for the first time.)")
            cuda_sources = [
                os.path.join(dcn_dir, "src", src_file)
                for src_file in ["deform_conv_cuda.cpp", "deform_conv_cuda_kernel.cu"]
            ]
            deform_conv_cuda = cpp_extension.load(
                name="deform_conv_cuda",
                sources=[
                    os.path.join(dcn_dir, "src", "deform_conv_cuda.cpp"),
