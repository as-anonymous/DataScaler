...
import torch
import yaml
from datascaler_open_clip.filter_utils.easyocr.DBNet.model.constructor import Configurable
...
class DBNet:
    ...
    def hmap2bbox(
        ...
        boxes_batch, scores_batch = zip(
            *[
                zip(*[(box, score) for (box, score) in zip(boxes, scores) if score > 0])
                if any(scores > 0)
                else [(), ()]
