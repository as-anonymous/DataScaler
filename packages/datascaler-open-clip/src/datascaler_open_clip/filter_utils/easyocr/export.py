...
def export_detector(
    ...
    if detector:
        ...
        print(
            f"torch outputs: y_torch_out.shape={y_torch_out.shape} feature_torch_out.shape={feature_torch_out.shape}"
        )
        print(
            f"onnx outputs: y_onnx_out.shape={y_onnx_out.shape} feature_onnx_out.shape={feature_onnx_out.shape}"
        )
        # compare ONNX Runtime and PyTorch results
        np.testing.assert_allclose(to_numpy(y_torch_out), y_onnx_out, rtol=1e-03, atol=1e-05)
        np.testing.assert_allclose(
            to_numpy(feature_torch_out), feature_onnx_out, rtol=1e-03, atol=1e-05
        )

        print(
            f"Model exported to {detector_onnx_save_path} and tested with ONNXRuntime, and the result looks good!"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "-d", "--dynamic", action="store_true", help="Dynamic  input output shapes for detector"
    )
    parser.add_argument(
        "-is",
        "--in_shape",
    ...
    parser.add_argument(
        "-m", "--model_storage_directory", type=str, help="model storage directory for craft model"
    )
    parser.add_argument(
        "-u", "--user_network_directory", type=str, help="user model storage directory"
    )
