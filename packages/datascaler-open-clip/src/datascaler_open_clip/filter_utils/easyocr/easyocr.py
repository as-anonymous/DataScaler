...
from datascaler_open_clip.filter_utils.easyocr.config import *
from datascaler_open_clip.filter_utils.easyocr.recognition import get_text
from datascaler_open_clip.filter_utils.easyocr.utils import (
    calculate_md5,
    diff,
    download_and_unzip,
    get_image_list,
...
class Reader(object):
    ...
    def getDetectorPath(self, detect_network):
        if detect_network in self.support_detection_network:
            ...
            if os.path.isfile(detector_path) == False:
                ...
                assert (
                    calculate_md5(detector_path)
                    == self.detection_models[self.detect_network]["md5sum"]
                ), corrupt_msg
                LOGGER.info("Download complete")
            elif (
                calculate_md5(detector_path) != self.detection_models[self.detect_network]["md5sum"]
            ):
                if not self.download_enabled:
            ...
            raise RuntimeError(
                "Unsupport detector network. Support networks are {}.".format(
    ...
    def detect(
        ...
        text_box_list = self.get_textbox(
            self.detector,
            img,
    ...
    def recognize(
        ...
        if detail == 0:
            ...
            if paragraph:
                return [
                    json.dumps(
                        {"boxes": [list(map(int, lst)) for lst in item[0]], "text": item[1]},
                        ensure_ascii=False,
                    )
                    for item in result
                ]
