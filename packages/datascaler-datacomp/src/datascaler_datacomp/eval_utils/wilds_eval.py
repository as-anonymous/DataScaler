...
import torch
import wilds.common.metrics
import wilds.common.utils
from wilds.common.grouper import CombinatorialGrouper
from wilds.common.metrics.all_metrics import F1, Accuracy, Recall
from wilds.datasets.wilds_dataset import WILDSDataset

from datascaler_datacomp.eval_utils.wds_eval import create_webdataset, evaluate_webdataset

warnings.filterwarnings("ignore", module=r"(wilds|clip_benchmark|sklearn|torch)")
...
class MyAccuracy(Accuracy):
    def _compute_group_wise(self, y_pred, y_true, g, n_groups):
        ...
        group_metrics, group_counts = _avg_over_groups(element_wise_metrics, g, n_groups)
...
def create_metadata_loader(task, data_root=None, dataset_len=None, batch_size=64, num_workers=4):
    dataset, _ = create_webdataset(task, None, data_root, dataset_len, batch_size, num_workers)
    # Load metadata (npy) and no images
    dataset.pipeline = dataset.pipeline[:5]  # This will break if webdataset changes
    dataset = dataset.to_tuple("cls", "npy")
    if dataset_len:
...
def evaluate_wilds_dataset(
    ...
    # Evaluate
    metrics, y_pred, y_target = evaluate_webdataset(  # type: ignore
        task,
        model_arch,
        model_path,
        data_root,
    ...
    metadata_loader = create_metadata_loader(task, data_root, dataset_len, batch_size, num_workers)
    # Check metadata
    y_array = []
    metadata_array = []
    ...
    metrics.update(wilds_evaluator.eval(y_pred, y_target, metadata)[0])  # type: ignore
...
class IWildCamEvaluator(WILDSEvaluator):
    def __init__(self, metadata):
        ...
        self._eval_grouper = CombinatorialGrouper(dataset=self, groupby_fields=(["location"]))

    def eval(self, y_pred, y_true, metadata, prediction_fn=None):
        metrics = [
            MyAccuracy(prediction_fn=prediction_fn),
...
class FMoWEvaluator(WILDSEvaluator):
    ...
    def eval(self, y_pred, y_true, metadata, prediction_fn=None):
        ...
        for group_idx in range(region_grouper.n_groups):
            ...
            group_metric = region_results[metric.group_metric_field(group_idx)]  # type: ignore
            group_counts = region_results[metric.group_count_field(group_idx)]  # type: ignore
            all_results[f"{metric.name}_{group_str}"] = group_metric
            all_results[f"count_{group_str}"] = group_counts
            if region_results[metric.group_count_field(group_idx)] == 0 or "Other" in group_str:  # type: ignore
                continue
            all_results_str += (
                f"  {region_grouper.group_str(group_idx)}  "
                f"[n = {region_results[metric.group_count_field(group_idx)]:6.0f}]:\t"  # type: ignore
                f"{metric.name} = {region_results[metric.group_metric_field(group_idx)]:5.3f}\n"  # type: ignore
            )
            region_metric_list.append(region_results[metric.group_metric_field(group_idx)])  # type: ignore
